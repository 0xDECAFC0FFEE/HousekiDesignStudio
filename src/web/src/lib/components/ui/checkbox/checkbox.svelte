<script>
	// The shadcn-svelte checkbox, with one addition (everything else is the CLI's file,
	// unchanged): `hover:border-primary` (2026-09-22, the user: "every button highlights on
	// hover" -- this had none). Unconditional rather than gated on the checked state: checked
	// already draws `data-checked:border-primary`, so hover is then a no-op there and a preview
	// of the same accent border everywhere else.
